        ...
        JButton button2 = new JButton(new AbstractAction("Generate Classes and Instances") {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    ...
                    // create hashmap for classes and its instances
                    HashMap<String, ArrayList<String>> classesMap = new HashMap<String, ArrayList<String>>();
                    ...
                    for (OWLClass cls : classes) {
                        ...
                        // add class name and list of instances to hashmap

                       Stream<OWLIndividual> a =  EntitySearcher.getIndividuals((OWLClass) cls, ontology);
                        for (OWLIndividual individual : a.collect(Collectors.toSet())) {
                            // get conver indiviaul to string
                            String individualName = individual.toString();
                            // set string between # and > to be the individual name
                            individualName = individualName.substring(individualName.indexOf("#") + 1, individualName.indexOf(">"));
                            // add individual name to list of instances
                            instances.add(individualName);
                        }

                        classesMap.put(className, instances);
                    }

                    // write all classes and its instances to file
                    FileWriter fw = new FileWriter("C:\\Users\\satya\\Desktop\\classes.txt");
                    BufferedWriter bw = new BufferedWriter(fw);
                    // adda heading to the file
                    bw.write("Classes and their instances\n");
                    bw.newLine();
                    // iterate through hashmap
                    for (String key : classesMap.keySet()) {
                        bw.write(key + ":   ");
                        for (String instance : classesMap.get(key)) {
                            bw.write(instance + ",  ");
                        }
                        bw.newLine();
                    }
